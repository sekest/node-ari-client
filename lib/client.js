...
var swagger = require('@bluegill/swagger-client');
...
Client.prototype._attachApi = function () {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var request_auth = "Basic " + new Buffer(self._connection.user + ":" + self._connection.pass).toString("base64");
    request({ url: ariUrl, headers : { "Authorization" : request_auth } }, function (err, response) {
      if (err || !response || response.statusCode !== 200) {
        if(err) {console.log(err)};
        if(response.statusCode) {console.log(response.statusCode+":"+response.statusMessage)};
        self.emit('APILoadError', "ARI Resource Connection Failed");
        reject("ARI Connection Failed");
      } else {
